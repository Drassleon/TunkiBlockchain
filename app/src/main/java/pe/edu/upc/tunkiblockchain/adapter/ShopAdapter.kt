...
class ShopAdapter(var list: List<CoinProvider>): RecyclerView.Adapter<ShopViewHolder>(){
    ...
    override fun onBindViewHolder(holder: ShopViewHolder, pos: Int) {
        ...
        when(list[pos].coinProviderName)
        {
            "Plaza Vea"->holder.shopProfileImageView.setImageResource(R.drawable.plaza_vea_logo)
            "Agente IBK Torre"->holder.shopProfileImageView.setImageResource(R.drawable.ibk_logo)
            "Cajero IBK Torre"->holder.shopProfileImageView.setImageResource(R.drawable.ibk_logo)
        }
